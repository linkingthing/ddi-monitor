...
func Register(conn *grpc.ClientConn, conf *config.MonitorConfig) {
	...
	_, err := cli.Register(context.Background(), &pb.RegisterReq{IP: conf.Server.IP, HostName: conf.Server.HostName, Roles: util.GetPbRoles(conf.Server.Roles), ControllerIP: s[0], Master: conf.Master})
	for err != nil {
		if _, err = cli.Register(context.Background(), &pb.RegisterReq{IP: conf.Server.IP, HostName: conf.Server.HostName, Roles: util.GetPbRoles(conf.Server.Roles), ControllerIP: s[0], Master: conf.Master}); err != nil {
